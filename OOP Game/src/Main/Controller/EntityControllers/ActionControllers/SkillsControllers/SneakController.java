...
public class SneakController {
    ...
    public void performSkill(UserActionEnum u) {
        switch (u) {
            case Skill1:
                if (validatePickPocket()) new PickPocket(sneak).activate();
                break;
            case Skill2:
              //  if (validateDetectTrap()) new DetectTrap().activate();
              //  if (validateRemoveTrap()) new RemoveTrap().activate();
                break;
            case Skill3:
               // if (validateCreep()) new Creep().activate();
                break;
            case Skill4:
                if (validateRangedWeapon()) new RangedWeapon(sneak).activate();
                break;
            default:

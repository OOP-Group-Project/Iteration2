...
public class SmasherController {
    ...
    public void performSkill(UserActionEnum u) {
        switch (u) {
            case Skill1:
                if (validateOneHanded()) new OneHandedWeapon(entity).activate();
                break;
            case Skill2:
                if (validateTwoHanded()) new TwoHandedWeapon(entity).activate();
                break;
            case Skill3:
                if (validateBrawling()) new Brawling(entity).activate();
                break;
            default:

...
public class TradeStateViewport extends StateViewport {
    ...
    private int ITEM_WIDTH = (ITEM_VIEW_WIDTH - (TradeState.SectionToShow.Inventory.getItemsPerRow() + 1) * ITEM_MARGIN) / InventoryState.SectionToShow.Inventory.getItemsPerRow();
    ...
    public TradeStateViewport(Viewport viewport, PlayStateViewport playStateViewport, TradeState tradeState){
    ...
    private void renderInventory(Graphics g){
        ...
        Inventory in = tradeState.getPlayerInventory();

        int size = in.getSize();
        Point selected = tradeState.getSelected();
